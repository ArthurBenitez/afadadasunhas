...
import { ArrowRight, CalendarHeart, GraduationCap, Heart, ShoppingBag, Star } from "lucide-react";
...
function HomePage() {
  ...
  const [products, setProducts] = useState<Array<{ id: string; name: string; image_url: string | null; price: number; external_url: string }>>([]);
  useEffect(() => {
    ...
    supabase.from('products').select('id,name,image_url,price,external_url').order('order', { ascending: true }).limit(4).then(({ data }) => {
      setProducts((data || []) as any);
    });
  }, []);
  const toAbsolute = (url: string) => /^https?:\/\//i.test(url) ? url : `https://${url}`;
  return (
    <>
      {/* HERO */}
      ...
      {/* PRODUTOS */}
      {products.length > 0 && (
        <section className="bg-gradient-bege py-16 md:py-24">
          <div className="mx-auto max-w-6xl px-4">
            <motion.div {...fadeUp} className="mb-10 flex flex-wrap items-end justify-between gap-4">
              <div className="max-w-2xl">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Produtos</p>
                <h2 className="mt-2 font-display text-4xl text-foreground md:text-5xl">Nossos favoritos.</h2>
                <p className="mt-3 text-muted-foreground">Itens selecionados pela Fada Das Unhas para você levar para casa.</p>
              </div>
              <Link to="/marketplace" className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline">
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {products.map((p, i) => (
                <motion.a
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 0.5, delay: i * 0.05 }}
                  className="group flex flex-col overflow-hidden rounded-2xl border border-border/70 bg-card shadow-soft transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-luxury"
                >
                  ...
                  <div className="flex flex-1 flex-col p-4">
                    <h3 className="font-display text-base text-foreground line-clamp-2">{p.name}</h3>
                    <div className="mt-auto flex items-center justify-between pt-4">
                      <span className="font-display text-lg text-primary">{brl(Number(p.price))}</span>
